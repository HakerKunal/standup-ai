...
@app.get("/health")
async def root():
    return {"status": "ok", "message": "AI Standup Generator API — Groq", "model": MODEL}
    ...
    def stream_generator():
        try:
            ...
            if "auth" in error_msg.lower() or "api_key" in error_msg.lower() or "401" in error_msg:
                msg = "Invalid Groq API key. Check your GROQ_API_KEY in the .env file."
            elif "rate" in error_msg.lower() or "429" in error_msg:
                msg = "Rate limit reached. Please wait a moment and try again."
            elif "model" in error_msg.lower() or "404" in error_msg:
                msg = "Model not found. Try changing MODEL to 'llama-3.1-8b-instant' in main.py."
            else:
